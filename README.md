...
- **Automated Compliance Checks**:
    ...
    - **Font Consistency**:
        - Headings: Checks for Arial, exactly 24pt, Bold, black.
        - Body Text: Checks for Arial, at least 16pt.
- **Interactive Web Interface**: A Streamlit-based UI to upload a deck, review issues per slide, and selectively apply fixes.
- **Change Reports**: Download the corrected deck plus a plain-text report listing every fix that was applied and everything that still needs manual attention in PowerPoint.
...
Upload a `.pptx` file. The app lists all issues grouped by slide — check the boxes for
the fixes you want to apply (or use "Select all"/"Clear all"), pick the deck's approved
marking variant if the auto-detected one is wrong, then click "Apply selected fix(es)" to
download the corrected deck and a change report.
...
**To mandate a specific marking variant** instead of auto-detecting it from the
majority of slides:
```bash
python3 slide_check.py your_presentation.pptx --marking not_cui_footer
python3 slide_check.py your_presentation.pptx --marking cui_header
```
...
| **Heading** | Arial, exactly 24pt, Bold, black | Yes |
...
## 📂 Project Structure
- `frontend/app.py`: Streamlit web application. `frontend/logo.png` is the header/favicon image.
- `.streamlit/config.toml`: The app's visual theme (colors, fonts, radii).
